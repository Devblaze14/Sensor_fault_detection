...
class ThresholdDetector(BaseDetector):
    """Simple bounds checking on raw signals."""
...
class ZScoreDetector(BaseDetector):
    """Rolling window statistical z-score."""
...
class KalmanResidualDetector(BaseDetector):
    """Uses the normalized innovation squared (NIS) or residual bounds from the Kalman filter."""
    def __init__(self, threshold: float = 9.0):
        # 3-sigma corresponds to threshold ~9.0 for 1DOF chi-square
        self.threshold = threshold
...
class AutoencoderDetector(BaseDetector):
    """PyTorch-based autoencoder trained on healthy data to flag anomalies based on reconstruction error."""
    def __init__(self, window_size: int = 20, hidden_dim: int = 4, threshold: float = 0.5):
        self.window_size = window_size
        self.threshold = threshold
    ...
    def train_model(self, healthy_data: np.ndarray, epochs: int = 50, batch_size: int = 32, lr: float = 0.001):
        ...
        windows = np.vstack([healthy_data[i:i + self.window_size] for i in range(num_windows)])
