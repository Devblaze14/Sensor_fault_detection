"""Evaluation engine for degradation detection performance.
Computes classification metrics, detection latency, and early-degradation
lead time against known ground-truth degradation onset."""
...
class Evaluator:
    """Evaluates detector performance against a known degradation onset."""

    def __init__(self, time_array: np.ndarray, ground_truth_idx: int):
        self.time = time_array
        self.gt_idx = ground_truth_idx
        # Binary ground truth: 0 before degradation, 1 after
    ...
    def evaluate(self, detections: np.ndarray, scores: np.ndarray = None) -> dict:
        """Evaluate a binary detection array against ground truth.
        ...
        if len(true_positives) > 0:
            ...
            alert_time = self.time[first_tp_idx]
        else:
            latency = float('inf')
            latency_samples = float('inf')
            alert_time = float('inf')
        ...
        # --- Early-degradation metrics ---
        degradation_start = self.time[self.gt_idx]
        lead_time = latency  # time between onset and first correct detection
        false_maint_alerts = int(fp)  # false positives before degradation
        ...
        metrics = {
            ...
            'Recall': recall,
            'F1': f1,
            'Degradation Start (s)': round(degradation_start, 4),
            'Alert Trigger (s)': round(alert_time, 4) if alert_time != float('inf') else float('inf'),
            'Lead Time (s)': round(lead_time, 4) if lead_time != float('inf') else float('inf'),
            'False Maintenance Alerts': false_maint_alerts,
        }
