...
def run_single_experiment(noise_std, severity, dt, duration=10, random_seed=42):
    ...
    # Optional: Generate a secondary healthy dataset purely for training the AE
    gen_train = IMUSignalGenerator(sampling_rate=int(1/dt), duration=20, noise_std=noise_std, random_seed=random_seed+1)
    train_healthy = gen_train.generate_noisy_signal()

    # 2. Inject Fault (e.g. Bias Drift)
    injector = FaultInjector(random_seed=random_seed)
    start_ratio = 0.5
    faulty_signal, gt_idx = injector.inject_bias_drift(noisy, start_ratio=start_ratio, drift_rate=severity)

    # 3. Setup Estimators and Detectors
    ...
    # Train Autoencoder
    ae_d = AutoencoderDetector(window_size=int(0.2/dt), threshold=noise_std**2 * 5)
    ...
    metrics, _ = evaluator.evaluate(np.array(hybrid_preds), np.array(hybrid_scores))
...
def run_experiments():
    ...
    print(df.groupby(['Noise', 'Detector']).mean()[['Latency (s)', 'F1 Score', 'FPR']])
